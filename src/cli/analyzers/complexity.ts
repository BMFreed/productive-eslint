...
const CANONICAL_RULE_SCORE_MAP: Record<string, number> = {
  ...
  'sonarjs/no-all-duplicated-branches': 3,
  'sonarjs/no-duplicated-branches': 3,
  'sonarjs/no-gratuitous-expressions': 2,
  'sonarjs/no-identical-conditions': 3,
  'sonarjs/no-identical-expressions': 3,
  'sonarjs/no-invariant-returns': 3,
...
const CANONICAL_RULE_REASON_MAP: Record<string, string> = {
  ...
  'sonarjs/no-all-duplicated-branches':
    'Every branch has the same implementation, so the control flow is likely misleading.',
  'sonarjs/no-duplicated-branches':
    'Duplicated branch implementations make it harder to infer the intended behavior.',
  'sonarjs/no-gratuitous-expressions':
    'A condition or expression is structurally unnecessary and can obscure intent.',
  'sonarjs/no-identical-conditions':
    'Repeated branch conditions usually point to stale or unreachable logic.',
  'sonarjs/no-identical-expressions':
    'Identical expressions in a comparison can turn branching into dead intent.',
  'sonarjs/no-invariant-returns':
    'A function returns the same value through different branches, making the branching suspect.',
  'sonarjs/no-nested-conditional':
    'Nested conditionals compress multiple branches into a hard-to-edit expression.',
}
...
const formatComplexityTarget = (finding: IAnalyzerFinding): string =>
  finding.symbol ? `${finding.file} :: ${finding.symbol}` : finding.file
...
export const analyzeComplexity = async (
  ...
  for (const result of results) {
    ...
    for (const [, entry] of groupedHits) {
      ...
      const symbol = namedOwner?.displayName ?? '<module>'
      ...
      findings.push({
        ...
        labels: [
          severity,
          ...(namedOwner ? [`function:${symbol}`] : ['module-scope']),
        ],
        ...(namedOwner ? { line: namedOwner.startLine } : {}),
  ...
  const firstFinding = firstSummary?.findings[0]

  return {
    fileCount: results.length,
    findingsCount: findings.length,
    nextStep: firstSummary
      ? `Start with \`${firstFinding ? formatComplexityTarget(firstFinding) : firstSummary.file}\` and reduce the branch and side-effect risk there first.`
      : 'No complexity hotspots were reported by the current analyzer ruleset.',
    suggestedOrder: summaries.flatMap((summary) =>
      summary.findings.map(formatComplexityTarget),
    ),
    summaries,
    summaryLines: [
